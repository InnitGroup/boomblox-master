...
<div id="ctl00_cphRoblox_ItemPurchasePopupPanel" class="modalPopup" style="z-index: 5; position: absolute; left: 38%; top: 25%; width: 27em; display: <?=isset($purchaseData) ? "block" : "none"?>">
    <div id="ctl00_cphRoblox_ItemPurchasePopupUpdatePanel">
        <?php if (isset($purchaseData) && !isset($_POST['ctl00$cphRoblox$ProceedWithTicketsPurchaseButton']) && !isset($_POST['ctl00$cphRoblox$ProceedWithRobuxPurchaseButton'])): ?>
        <div id="VerifyPurchase_<?=$currencyName?>" style="margin: 1.5em;">
            <h3>Purchase Item:</h3>
            <p>Would you like to purchase <?=$data->catalogType?> "<?=htmlspecialchars($data->itemName)?>" from <?=$data->creatorName?> for <?=$shortName?> <?=number_format($price)?>?</p>
            <p>Your balance after this purchase will be <?=$shortName?> <?=number_format($balance)?>.</p>
            <p><input type="submit" name="ctl00$cphRoblox$ProceedWith<?=$currencyName?>PurchaseButton" value="Buy Now!" onclick="document.getElementById('VerifyPurchase_<?=$currencyName?>').style.display = 'none';document.getElementById('ProcessPurchase_<?=$currencyName?>').style.display = 'block';" id="ctl00_cphRoblox_ProceedWithTicketsPurchaseButton" class="MediumButton" style="width:100%;"></p>
            <p><input type="submit" name="ctl00$cphRoblox$Cancel<?=$currencyName?>PurchaseButton" value="Cancel" onclick="$find('myBehavior1').hide();" id="ctl00_cphRoblox_Cancel<?=$currencyName?>PurchaseButton" class="MediumButton" style="width:100%;"></p>
        </div>
...
<?php 
if (isset($_POST['ctl00$cphRoblox$ProceedWithTicketsPurchaseButton']) || isset($_POST['ctl00$cphRoblox$ProceedWithRobuxPurchaseButton'])) {
