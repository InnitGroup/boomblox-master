...
<div id="MainPanel">
    <div id="Report" style="border:solid 1px black; background-color:white; padding:15px; width:fit-content; text-align:center;">
        <?php if ($abuse["type"] !== "chat"): ?>
            <?php 
            $thumb;
            $render;
            $abuse["type"] == "user" ? $thumb = new Avatar($abuse["abuse"]) : $thumb = new Asset($abuse["abuse"]);
            $abuse["type"] == "user" ? $render = $thumb->GetThumbnail(500, 500, "PNG") : $render = $thumb->getThumbnail(250, 250, "PNG");
            ?>
            <img src="<?=$render?>" style="height:250px;">
        <?php endif; ?>
        <p>Abuse ID: <?=$abuse["id"]?></p>
        <p>Type: <?=ucfirst($abuse["type"])?></p>
        <p>Reported Abuse: <a href="<?=$reportedAbuse?>" target="_blank"><?=ucfirst($abuse["type"])?></a></p>
